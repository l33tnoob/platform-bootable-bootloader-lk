...
/* name: should be translated to official name first in security library */
static part_t *sec_mt_part_get_partition(char *name)
{
    part_t *part = NULL;
    int found = 0;
    SEC_PART_TRANS_TBL_ENTRY *entry = g_part_name_trans_tbl;
    ...
    if (found) {
        part = mt_part_get_partition(entry->official_name);
        if (part) {
            return part;
        }

        /* try alt_name1 */
        part = mt_part_get_partition(entry->alt_name1);
        if (part) {
            return part;
        }

        /* try alt_name2 */
        part = mt_part_get_partition(entry->alt_name2);
        if (part) {
            return part;
        }

        /* try alt_name3 */
        part = mt_part_get_partition(entry->alt_name3);
        if (part) {
            return part;
        }
    }
...
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
    part_t *part;
    part = sec_mt_part_get_partition(part_name);
    if (!part){    
        dprintf(CRITICAL,"[read_wrapper] sec_mt_part_get_partition error, name: %s\n", part_name);
        ASSERT(0);
    }
#endif
#endif
...
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
    len = dev->read(dev, read_addr, (uchar *) data, size, part->part_id);
#else
...
int sec_dev_write_wrapper(char *part_name, u64 offset, u8* data, u32 size)
{
    ...
    part = sec_mt_part_get_partition(part_name);
    if (!part){    
        dprintf(CRITICAL,"[write_wrapper] sec_mt_part_get_partition error, name: %s\n", part_name);
        ASSERT(0);
    }
#endif
#endif
...
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
    len = dev->write(dev, (uchar *) data, write_addr, size, part->part_id);
#else
