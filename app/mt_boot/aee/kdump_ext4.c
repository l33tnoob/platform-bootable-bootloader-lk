...
{   
    ...
    // check if dumpsize > allocated file size, then don't write to device
    if(handle->allocsize < 0)
        return 0;
    ...
    if((buf == NULL) && (size == 0)) {
        if(!handle->dumpdev->write(handle->dumpdev, handle->wlba, handle->data, handle->midx)) {
            voprintf_error(" SDCard: Write dump data failed.\n");
            handle->allocsize = -1; // set the error flag by allocsize
            return 0;
        }
        return 0;
    }
...
int kdump_ext4_output(struct mrdump_control_block *mrdump_cb, uint32_t total_dump_size, struct mrdump_dev *mrdump_dev)
{
    ...
    if (handle->allocsize < 0) {
        ...
        if(handle->allocsize < -1) {
            voprintf_error(" dump size > allocated size. Abort!\n");
        } else {
            voprintf_error(" SDCard: Read and Write error.\n");
        }
    ...
    // Should ack write_cb the end of the file...
    lba_write_cb(handle, NULL, 0);
    ...
    if (ok) {
        ...
        mrdump_status_ok("%s:%s\nMODE:%s\nFILENAME:%s\n", mrdump_dev->name, "ext4", mrdump_mode2string(mrdump_cb->crash_record.reboot_mode), "FRDump.zip");
    }
